...
rm(list=ls())
#Set el directorio para trabajar 
...
normalidad <- sapply(genes, function(g){
  x <- df[[g]]; x <- x[!is.na(x)]   # revisar si tenemos algun NA
  x <- df[[g]]
  if(length(x) > 50){
    p <- ad.test(x)$p.value     # prueba Anderson-Darling para N >= 50
  } else {
    p <- shapiro.test(x)$p.value # Shapiro Wilk para N < 50
  }
  return(p >= 0.05)   # si TRUE = normal
})
...
normalidad_summ <- data.frame(
  ...
  Test_utilizado = ifelse(sapply(genes, function(g) length(df[[g]]) > 50),
                          "Anderson-Darling", "Shapiro-Wilk"),  # incluir en tabla test
  ...
  Interpretación = ifelse(normalidad, 
                          ...
                          "No normal")   #interpretacion
)
normalidad_summ

```

2.  Calcular y analizar si no lo son de las variables bioquímicas, síntomas, sociodemográficas y de los genes:
...
norm_parametrico <- sapply(parametrico, function(g){
  x <- df[[g]]; x <- x[!is.na(x)]
  if(length(x) > 50){
    p <- ad.test(x)$p.value    # Anderson-Darling para N > 50
  } else {
    p <- shapiro.test(x)$p.value #Shapiro Wilkins para N < 50
  }
...
func_parametricos <- lapply(norm_parametrico, function(is_normal) {
  if (is_normal)
    ...
    "{median} ({p25}–{p75})"  # mediana + rango intercuartílico (p25-p75) si no lo son
})
...
## ---------- Modelo Tabla 2 ----------- ##
# Tabla para mostrar las variables que son numericas 
tabla_parametricos <- df %>%
  ...
  tbl_strata(
    ...
    .tbl_fun = ~ .x %>%
      ...
      add_p(
        test = all_continuous() ~ "kruskal.test",  # Kruskal–Wallis porque la                                                               distributcion no es normal
        pvalue_fun = ~ style_pvalue(.x, digits = 3)
      )
  )

# Tabla para mostrar las variables que son categoricas
tabla_categoricos <- df %>%
  ...
  tbl_strata(
    ...
    .tbl_fun = ~ .x %>%
      tbl_summary(
        by = extension,
        statistic = all_categorical() ~ "{n} ({p}%)" #porcentage para describir a                                                           variables categoricas 
      ) %>%
      add_p(
        test = all_categorical() ~ "chisq.test",  #Chi test para variables categoricas
        pvalue_fun = ~ style_pvalue(.x, digits = 3)
      )
  )
...
Las estadísticas descriptivas de la tabla de variables categóricas podría estar incorrectas puesto que en algunos de los grupos de extensión solo hay un valor posible. La tabla se muestra pero no fue analizada, ni tomada en cuenta para futuras descripciones. 
...
p50 <- median(df$edad)  #mediana de la edad

df <- df %>%
  mutate(edad_cat = ifelse(edad < p50, "Edad < P50", "Edad ≥ P50") %>% factor()) #aumentar una columna con la informacion de la edad. 

#Si los genes cumplen con distribucion normal
...
tabla_modelo3 <- df %>%
  ...
  add_p(
    test = all_continuous() ~ "wilcox.test", #wilcox test porque no cumple con normal.
    pvalue_fun = ~ style_pvalue(.x, digits = 3)
  )
